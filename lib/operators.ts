 ...
 */
export function loop( iterations = 1, cb: (index: number) => void, index = 0, step = 1) {
  for (index; index < iterations; index += step) {
    cb(index);
  }
}
...
export function objloop<Obj extends object>(
  obj: Obj,
  cb: (key: keyof Obj, val: Obj[keyof Obj]) => void
) {
 ...
 */
export function keyloop<Obj extends object>(
  obj: Obj,
  cb: (key: keyof Obj) => void
) {
  for (const key in obj) {
    cb(key);
  }
 ...
 */
export function valloop<Obj extends object>(
  obj: Obj,
  cb: (val: Obj[keyof Obj]) => void
) {
  for (const key in obj) {
    cb(obj[key]);
  }
