...
Array.prototype.bubbleSort = function (order = "asc") {
    ...
    for (let i = 0; i < this.length; i++) {
        for (let j = 0; j < this.length - i - 1; j++) {
            const a = this[j]; // Type assertion to ensure that a is not null or undefined
            const b = this[j + 1]; // Type assertion to ensure that b is not null or undefined
...
Array.prototype.quickSort = function (order = "asc") {
    ...
    const quickSortRecursion = (arr, left, right) => {
        if (left < right) {
...
Array.prototype.unique = function () {
    ...
    if (this.every((item) => typeof item === "string"))
        return [...new Set(this.map((item) => item.toLowerCase()))];
    return [...new Set(this)];
};
...
Array.prototype.binarySearch = function (target, sortalgo) {
    ...
    switch (sortalgo) {
        ...
        case "counting":
            ...
            sortedArray = this.map(num => num).countingSort();
            break;
        case "radix":
            if (!this.every((num) => typeof num === "number")) {
                throw new Error("Radix Algorithm supports only arrays of numbers");
            }
            sortedArray = this.map(num => num).radixSort();
            break;
        default:
            sortedArray = this.sort(); // Add type assertion here
            break;
    }
    // Binary search
    let start = 0;
...
Array.prototype.sample = function (quantity = 1) {
    if (this.length === 0)
        return this;
    if (quantity <= 1 || quantity > this.length)
...
Object.assign(JSON, {
        ...
        for (const key in json) {
            if (Object.prototype.hasOwnProperty.call(json, key)) {
                ...
                if (typeof json[key] === "object" && !Array.isArray(json[key])) {
                    ...
                    const nested = JSON.flatten(json[key], newKey);
    ...
    unflatten: (json) => {
        ...
        for (const key in json) {
            ...
            if (typeof json[key] === "object") {
                cur[keys[keys.length - 1]] = JSON.unflatten(json[key]);
            }
            else {
        ...
        for (const key in json) {
            if (Object.prototype.hasOwnProperty.call(json, key)) {
                ...
                if (typeof value === "object" && !Array.isArray(value)) {
                    ...
                    const nested = JSON.filter(value, condition);
    ...
    sort: (json, key, order = "asc") => {
        return json.sort((a, b) => {
            const valueA = a[key];
            const valueB = b[key];
            if ((valueA === valueB) || ((!valueA || valueA === null) && (!valueB || valueB === null)))
                return 0;
    ...
    validateSchema: (data, schema) => {
        function validateObject(data, objectSchema) {
        ...
        function validateProperty(value, propertySchema) {
            ...
            if (propertySchema.type === Object && propertySchema.properties) {
                if (!validateObject(value, propertySchema.properties))
                    return false;
            }
...
Date.prototype.timestamp = async function (timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone) {
    return new Date((await axios$1.get(`https://script.google.com/macros/s/AKfycbyd5AcbAnWi2Yn0xhFRbyzS4qMq1VucMVgVvhul5XqS9HkAyJY/exec?tz=${timeZone}`)).data.fulldate);
};
